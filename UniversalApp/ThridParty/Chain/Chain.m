...
NSString *const kChainErrorDomain = @"com.typzq.sskeychain";

NSString *const kChainAccountKey = @"acct";
NSString *const kChainCreatedAtKey = @"cdat";
NSString *const kChainClassKey = @"labl";
NSString *const kChainDescriptionKey = @"desc";
NSString *const kChainLabelKey = @"labl";
NSString *const kChainLastModifiedKey = @"mdat";
NSString *const kChainWhereKey = @"svce";

#if __IPHONE_4_0 && TARGET_OS_IPHONE
CFTypeRef ChainAccessibilityType = NULL;
#endif
...
+ (NSArray *)accountsForService:(NSString *)service error:(NSError **)error {
    OSStatus status = ChainErrorBadArguments;
    ...
    if (status != noErr && error != NULL) {
        *error = [NSError errorWithDomain:kChainErrorDomain code:status userInfo:nil];
        return nil;
    }
...
+ (void)setAccessibilityType:(CFTypeRef)accessibilityType {
    ...
    if (ChainAccessibilityType) {
        CFRelease(ChainAccessibilityType);
    }
    ChainAccessibilityType = accessibilityType;
}
#endif
