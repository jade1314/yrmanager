...
- (NSMutableArray *)yearsInRange {
    ...
    for (NSInteger i=yearStart; i<=yearEnd; i++) {
        SolarDate solarDate=SolarDate((int)i, 1, 1);
        ChineseDate chineseDate;
        solarDate.ToChineseDate(chineseDate);
        std::pair<int, int> p=ChineseCalendarDB::GetEraAndYearOfLunar((int)i);
        //我们要把农历的年份显示为甲寅年这样的文字，所以years容器中应该存储甲子年份，但是农历的甲子年份60年循环一次，无法与公历年份对应，所以我们额外存储年代、公历年份
        [array addObject:[NSString stringWithFormat:@"%d-%d-%ld", p.first, p.second, (long)i]];
    }
    return array;
}
