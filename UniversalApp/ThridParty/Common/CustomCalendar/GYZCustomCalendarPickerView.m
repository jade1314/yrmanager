...
@implementation GYZCustomCalendarPickerView{
    ...
    IDJCalendar *cal;//日期类
    NSMutableArray *defaultDateArr;
}

- (instancetype)initWithTitle:(NSString *)title andDefaultDate:(NSString *)date
{
    ...
    if (self)
    {
        ...
        if (date) {
            //拆分
            NSMutableString *mutDate = date.mutableCopy;
            date = [mutDate stringByReplacingOccurrencesOfString:@"-" withString:@"."];
            NSArray *array = [date componentsSeparatedByString:@"."];
            
            defaultDateArr = (NSMutableArray *)array;
            //检测数据
            NSString *two = defaultDateArr[1];//月
            if (two.length == 2 && [two intValue] < 10) {
                two = [two substringFromIndex:1];
                [defaultDateArr replaceObjectAtIndex:1 withObject:two];
            }
            NSString *three = defaultDateArr[2];//日
            if (three.length == 2 && [three intValue] < 10) {
                three = [three substringFromIndex:1];
                [defaultDateArr replaceObjectAtIndex:2 withObject:three];
            }
            
            for (NSString *content in defaultDateArr) {
                if ([ToolHelper isBlankString:content]) {
                    [defaultDateArr removeAllObjects];//防止有脏数据
                }
            }
        }
...
-(void)setCalendarType:(CalendarType)calendarType{
    ...
    if (calendarType == GregorianCalendar) {
        if (defaultDateArr && defaultDateArr.count == 3) {
            [self _setDaysInMonth:defaultDateArr[1] year:defaultDateArr[2]];
            [_pickerView selectRow:[years indexOfObject:defaultDateArr[0]] inComponent:0 animated:YES];
            [_pickerView selectRow:[months indexOfObject:defaultDateArr[1]] inComponent:1 animated:YES];
            [_pickerView selectRow:[days indexOfObject:defaultDateArr[2]] inComponent:2 animated:YES];
            cal.year = defaultDateArr[0];
            cal.month = defaultDateArr[1];
            cal.day = defaultDateArr[2];
        }else{
