...
static FileBMS *instance=nil;  //单例对象

@implementation FileBMS
...
- (BOOL)createFile:(NSString *)fileName overwrite:(BOOL)shouldOverwrite {
    ...
    @try {
        ...
        /*
         ...
         lastTag = [fileName rangeOfString:@"/" options:NSBackwardsSearch];
         if (lastTag.location != NSNotFound && lastTag.location != 0) {
         shortDir = [fileName substringToIndex:lastTag.location];
         fullDir = [self fullFileName:shortDir];
         ...
         bRet = [fileManager createFileAtPath:file contents:nil attributes:nil];
         NSLog(@"创建文件 (%@) %@", file, bRet ? @"成功" : @"失败");
         }
         */
        //只有文件不存在或者可以覆盖时，才执行创建
