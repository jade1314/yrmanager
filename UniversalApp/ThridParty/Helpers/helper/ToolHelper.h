...
enum _DateHelperTimeTextType {
    DateHelperTimeTextTypeCut= -1,
    DateHelperTimeTextTypeColon,
    DateHelperTimeTextTypeNone,
    DateHelperTimeTextTypeYMD,
    DateHelperTimeTextTypeYMDNone,
    DateHelperTimeTextTypeDefault
};

enum _DateHelperTimeForShowForm {
    DateHelperTimeForShowFormList= 1,
    DateHelperTimeForShowFormDetail
};
...
//为字符串加格式
+(NSString *)formattingString:(NSString *)detailStr withFont:(NSString *)font;

//显示提示
+(void)showPrompt:(NSString *)message andOriginController:(UIViewController *)origin andSetBackgoundUserInteractionEnabled:(BOOL)enabled andBlock:(void(^)(void))getBlock;
+ (void)alertWarning:(NSString *)message fromController:(UIViewController *)fromVC backgoundTapDismissEnable:(BOOL)enabled andLeftBlock:(void(^)(void))leftBlock andRightBlock:(void(^)(void))rightBlock;
+ (void)alertWarningTip:(NSString *)message leftText:(NSString *)leftText rightText:(NSString *)rightText fromController:(UIViewController *)fromVC backgoundTapDismissEnable:(BOOL)enabled andLeftBlock:(void(^)(void))leftBlock andRightBlock:(void(^)(void))rightBlock;
//获取时间间隔
+(NSInteger)getDaysFrom:(NSDate *)serverDate To:(NSDate *)endDate;
//删除某一文件
+(BOOL)deleteLocalFile:(NSString *)path;
//判断当前文件是否存在
+(BOOL)fileIsExist:(NSString *)path;
//判断是否是数字
+ (BOOL)isNumText:(NSString *)str;
//云之道加密
+(NSString *)jiaPlusmi:(NSString *)text;
//云之道解密
+(NSString *)jieSubmi:(NSString *)text;

/* --------------------------------- 股票数据格式相关  --------------------------*/
//股票6位转成8位
+(NSString *)judgeStockCode:(NSString *)code market:(NSString *)market;
//沪市6位转8位
+(NSString*)huStockCode2:(NSString *)code_2 andCode3:(NSString *)code_3 withCode:(NSString *)code;
//深市6位转8位
+(NSString*)shenStockCode2:(NSString *)code_2 andCode3:(NSString *)code_3 withCode:(NSString *)code;
//判断固定字体大小
+(CGRect)stringRectWithSize:(CGSize)size fontSize:(CGFloat)font withString:(NSString *)string;
//判断字体大小  (根据字体类型)
+(CGRect)stringRectWithSize:(CGSize)size fontSize:(CGFloat)font withString:(NSString *)string andStringType:(NSString *)type;
//判断富文本大小
+(CGRect)stringRectWithSize:(CGSize)size withString:(NSString *)string WithDic:(NSDictionary *)attri;
...
+(NSString *) getNowDate;
+ (void)getNowTimeSuccess:(void (^)(NSString *responseObject)) success;
/**
 * 获取几天前或几天后的日期
...
//寻找图片的不同的点
+(CGPoint)divisiveColor:(UIImage*)image;

//*8位行情代码转交易市场代码*/
+(NSString *)tradeMarketWithMarketCode:(NSString *)code;
...
+ (BOOL)is24HourCreateDate:(NSDate *)createDate toDate:(NSDate *)now;
+(NSString *)getMMSSFromSS:(NSInteger)totalTime;
+(NSString *)getddMMFromSS:(NSInteger)totalTime;
