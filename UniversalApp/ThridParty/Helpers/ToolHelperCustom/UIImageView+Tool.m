...
-(void)getGifImageWithUrk:(NSURL *)url returnData:(void(^)(NSArray<UIImage *> * imageArray, NSArray<NSNumber *>*timeArray,CGFloat totalTime, NSArray<NSNumber *>* widths,NSArray<NSNumber *>* heights))dataBlock{
    ...
    for (size_t i=0; i<count; i++) {
        ...
        CFRelease((__bridge CFDictionaryRef)(info));
    }
    CFRelease(source);
...
//解析gif文件数据的方法 block中会将解析的数据传递出来
-(void)getImagesWithUrk:(NSArray *)images returnData:(void(^)(NSArray<UIImage *> * imageArray, NSArray<NSNumber *>*timeArray,CGFloat totalTime, NSArray<NSNumber *>* widths,NSArray<NSNumber *>* heights))dataBlock{
...
-(void)PlayImages:(NSArray *)images{
    ...
    __weak id __self = self;
    [self getImagesWithUrk:images returnData:^(NSArray<UIImage *> *imageArray, NSArray<NSNumber *> *timeArray, CGFloat totalTime, NSArray<NSNumber *> *widths, NSArray<NSNumber *> *heights) {
        //添加帧动画
        CAKeyframeAnimation *animation = [CAKeyframeAnimation animationWithKeyPath:@"contents"];
        NSMutableArray * times = [[NSMutableArray alloc]init];
        float currentTime = 0;
