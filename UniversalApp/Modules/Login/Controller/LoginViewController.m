...
-(UIView*)alipayView:(CGRect)frame{
    ...
    [alipay addEventHandler:^(UIButton* sender) {
        ...
        if ([apiUrl hasPrefix:@"https://pay.example.com"]) {
            K_Log(@"请部署pay.php，填写自家的key。否则无法测试。");
        }else{
            //网络请求不要阻塞UI，仅限Demo
...
-(void)qqViewHandler:(UIButton*)btn{
    ...
    if (btn.tag>=2) {
        msg.image=UIImagePNGRepresentation(testImage);;
        msg.thumbnail=UIImagePNGRepresentation(testThumbImage);
...
//            [self jumpToBizPay];
    ...
    if (btn.tag==30002) {
        ...
        msg.image=UIImagePNGRepresentation(testThumbImage);//新闻类型的职能传缩略图就够了。
        ...
        msg.image = UIImagePNGRepresentation(testImage); //高清图，小于128K
        msg.thumbnail = UIImagePNGRepresentation(testThumbImage); //6.5.9及以上版本微信客户端小程序类型分享使用大图卡片样式，使用WXMiniProgramObject的hdImageData属性展示高清大图；旧版微信客户端（6.5.8及以下版本）小程序类型消息卡片使用小图卡片样式，拉取thumbData字段图片信息。请开发者注意兼容旧版客户端。 小于32K
